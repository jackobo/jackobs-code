...
namespace Spark.Infra.Types
{
    public class VersionNumber : ICloneable, IComparable, IComparable<VersionNumber>, IEquatable<VersionNumber>, IXmlSerializable
    {
        ...
        const int MAX_VERSION = 9999;

        public VersionNumber GetNextMinorVersion()
        {
            if (this.Minor == MAX_VERSION)
                throw new InvalidOperationException($"Maximum Minor version ({MAX_VERSION}) was reached!");

            return new VersionNumber(this.Major, this.Minor + 1, 0, 0);
        }

        public VersionNumber GetNextRevisionVersion()
        {
            if (this.Revision == MAX_VERSION)
            {
                return GetNextMinorVersion();
            }

            return new VersionNumber(this.Major, this.Minor, this.Revision + 1, 0);
        }
